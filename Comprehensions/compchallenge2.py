...
neighbors = [
    [
        (navOptions[0], locations[navOptions[0]])
        for navOptions in exits.items()
        if loc in navOptions[1].values()
    ]
...
for destination in locations:
    neighbors = []
    for navOptions in exits.items():
        if destination in navOptions[1].values():
            neighbors.append((navOptions[0], locations[navOptions[0]]))
